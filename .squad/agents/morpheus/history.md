...
### 2026-07-15 (same day, follow-up 2): THIRD change — replace the regex classifier with the LLM (Ahmed directive)

Ahmed: "It shouldn't be a regex at all, we have the LLM for that." Replaced the entire keyword/regex `ClassifyOutcomeSpecReply` (affirmation vocab + clause splitter + clarification markers + normalizer) with an LLM-backed classifier.

**What:** New `OutcomeSpecReplyClassifier.cs` — `IOutcomeSpecReplyClassifier` + production `CopilotOutcomeSpecReplyClassifier`, mirroring the existing `CopilotWorkflowSelectionModel` lightweight-classification convention (single tool-less Copilot completion, no sandbox/session store, token scope via `IGitHubTokenScopeProvider`, client via `GitHubCopilotClientFactory`). Grounded in the proposed spec; returns `{"decision":"confirm"|"revise"}`. Fast/cheap model choice: `Generation:ReplyClassificationModel` override else shared Copilot model (low-latency synchronous steering path). `CoordinatorSteeringService` now delegates via `ClassifyOutcomeSpecReplyAsync` and **fails closed to Revise** when the classifier is absent / returns null / throws — a transient model outage can never silently confirm. `ParseDecision` trusts only a well-formed JSON `decision` field (else null); untrusted reply text fenced in the prompt.

**Revise-path stall (Ahmed re-flag):** Confirmed real, already fixed by the drain (1ca944f4) — confirm and revise both funnel through `SubmitDecisionAsync`/`DeferredDecisions`, so the drain covers BOTH kinds. LLM (right decision) + drain (decision applied) are complementary; both required.

**Validation:** Build clean 0/0. 90 coordinator/classifier/steering tests pass incl. new `OutcomeSpecReplyClassifierTests` + endpoint tests (classifier-driven confirm; fail-closed-to-revise on classifier outage) using a hermetic `FakeOutcomeSpecReplyClassifier` (no live model call). Full suite 2257 passed; only 5 pre-existing PodLocalWorkspaceManager Windows-path failures (confirmed on baseline).

**Status:** Committed as 9e6d0062 (supersedes the 84afd8aa regex). Branch `fix/coordinator-steer-confirm-272` now has 3 commits (1ca944f4 drain, 84afd8aa regex[superseded], 9e6d0062 LLM). No PR. Decision persisted via squad_decide. Ready for coordinator merge + live harness re-run.
