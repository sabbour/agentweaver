...
public interface IProjectWorkspaceProvider
{
    ...
    /// </summary>
    string ResolveWorkingDirectory(ProjectId id, string configuredPath);

    /// <summary>
    /// Ensures the workspace directory is present and accessible.
    /// Throws InvalidOperationException if the workspace cannot be made available
    /// (e.g. a persistent volume mount is missing in a cloud deployment).
    /// </summary>
    Task EnsureWorkspaceAsync(ProjectId id, string workingDirectory, CancellationToken ct = default);
