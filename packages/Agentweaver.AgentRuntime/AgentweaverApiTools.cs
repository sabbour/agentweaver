...
internal static class AgentweaverApiTools
{
        ...
        string? runId = null)
    {
        ...
        // up for them — even though PreviewRunnerToolProvider's `observe_bound_port` (registered
        // unconditionally for every sandboxed agent) tells the model to call it next. `start_preview`
        // now lives solely in PreviewRunnerToolProvider (apps/Agentweaver.AgentHost/PreviewRunner.cs),
        // which is always present alongside the other three preview lifecycle tools, so the two
