...
| 10 | `project-dashboard.png` | `projects.md` | `/projects/:projectId` | Open a project | Dashboard with project status, active-run summary, Activity range filter, Runs over time, Run totals, Model metrics, and Agent leaderboard. | PROJECT_ID; ≥1 run preferred |
| 11 | `dashboard-token-usage.png` | `token-usage-monitoring.md` | `/projects/:projectId` | Dashboard → Activity range | Project dashboard usage surfaces: range selector, Model metrics panels, Agent leaderboard Cost column, and AI-credit/token data when available. | PROJECT_ID; token/model telemetry |
...
| 44 | `cluster-page.png` | `cluster-page.md` | `/projects/:projectId/cluster` | Click Cluster | Cluster page with Orphaned, Pending capacity, Checks OK, Warm pool KPI cards, Health checks, Sandbox claims, orphaned pods, pending capacity, and warm pools. | Kubernetes cluster diagnostics/live pods |
...
| 47 | `observability-overview.png` | `token-usage-monitoring.md` | `/projects/:projectId/observability` | Click Observability | Project Observability Overview tab with range selector, Refresh, and model panels for calls, AI credits, usage, response time, and time to first token. | App Insights/model telemetry |
