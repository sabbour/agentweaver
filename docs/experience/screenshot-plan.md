...
| 8 | `create-from-github-dialog.png` | `projects.md` | `/projects` | Click **Create from GitHub** | **Create project from GitHub** dialog: **Name**, repository search/recent/repository-sources/paste controls (personal `@{login}` with **You**, then orgs), auto-filled **Repository folder**, and the **Suggested** / **Templates** / **Generate** blueprint tabs; or **Connect GitHub** when unauthenticated. |
...
| 13 | `workflow-run-graph.png` | `runs-board-watch.md` | `/projects/:projectId/orchestrations/:runId` | Open a coordinator run's topology | Embedded live graph with cost/status badges, preview action (when k8s sandbox + active run), and approval/question affordances. |
| 14 | `sandbox-preview-dialog.png` | `runs-board-watch.md` | `/projects/:projectId/orchestrations/:runId` | Click **Preview Sandbox** (k8s sandbox, active run) | **Sandbox Preview** dialog: "Preview traffic is proxied through the Agentweaver API server.", "Preview active for port {port} on pod {pod_name}.", **Cancel** / **Start preview** / **Stop preview** / **Close**. |
| 15 | `watch-timeline.png` | `runs-board-watch.md` | `/projects/:projectId/orchestrations/:runId` | Select a child/session detail | Embedded run timeline with stream status (**Connecting**/**Streaming**/**done**/**error**), turn groups, message bubbles, tool-call and lifecycle cards. |
...
| 26 | `per-run-workflow-graph.png` | `workflows-backlog.md` | `/projects/:projectId/orchestrations/:runId` | Open a coordinator run's embedded graph | Embedded graph with status badges (**Pending**/**In Progress**/**Complete**/**Skipped**/**Failed**/**Revise**/**Awaiting**), role labels, and node actions (**Review now** / **Browse files** / **View memories**); loopback edges on revision. |
...
| 38 | `run-pending-capacity.png` | `operations.md` | `/projects/:projectId/orchestrations/:runId` | Open an active coordinator run with at least one subtask in `PendingCapacity` | Coordinator topology graph with one or more subtask nodes showing the amber **Waiting for capacity** badge. |
...
| 39 | `watch-token-counter.png` | `experience/token-usage-monitoring.md` | `/projects/:projectId/orchestrations/:runId` | Run in progress | Shows token/cost context on embedded run inspection surfaces |
