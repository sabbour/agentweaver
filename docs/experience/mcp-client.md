...
| `project_create` | Creates a project from a local working directory, optionally clones a Repo App-authorized GitHub repository via `origin` + `repository_selection_code`, can apply either a predefined `blueprint_id` or an inline `blueprint`, and can carry generated workflow YAML. |
