...
- `apps/Agentweaver.Api/Assistant/RemoteOperatorAssistantAgent.cs` — per-turn AgentHost launch and A2A proxy.
...
Each assistant endpoint extracts the bearer presented on that specific HTTP request. `AssistantRunService` passes it only in the in-memory turn request; it is not written to the run row, history, or event log. `RemoteOperatorAssistantAgent` sends it to the freshly claimed AgentHost through the one-time internal `/configure` call, separately from the linked GitHub access token.
...
Durable rehydration (the mechanism described above) is unaffected by this and remains the correct answer to cross-pod/idle/restart continuity — it works entirely from Agentweaver's own event log, independent of the short-lived AgentHost or SDK session. Using the SDK's native store would require a deliberately reusable runtime and deterministic `SessionId` resume instead of creating a new session per turn; the current one-pod-per-turn path has no such state to preserve.
