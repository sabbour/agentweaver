...
3. For pod-per-run AgentHost pods, `GetPodIpAsync` then polls the pod's `status.podIP`, calls `POST /configure`, waits for `/healthz`, and registers the A2A endpoint `http[s]://<podIP>:<port><a2aPath>`.
