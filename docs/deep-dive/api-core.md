...
- **Checkpoint startup must be defensive.** Corrupt checkpoint metadata should be quarantined or ignored when possible so the API can start and recover. Under multiple replicas sharing a checkpoint volume, the exclusive store lock means only one pod opens the shared store; the others must fall back to a per-pod directory (keyed by `POD_NAME`) rather than crash-loop — `ResilientCheckpointStore.Create` must never throw. Expected, non-corruption failures (lock contention; shared-volume permission denial) fall back **quietly** — at most one `warn` per store, no `fail`/stacktrace — so they do not flood the logs at every startup; only genuine index corruption logs loudly and quarantines.
