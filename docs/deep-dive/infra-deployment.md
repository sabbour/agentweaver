...
The API reads GitHub OAuth client settings and the OAuth signing key from CSI-mounted files. The MCP server mounts no secrets — its auth relies only on OAuth (Agentweaver-minted JWT + transitional GitHub passthrough). The `agentweaver-api` (and `agentweaver-mcp`) service accounts use the Key-Vault-privileged managed identity (`agentweaver-api-identity`, federated credential `agentweaver-api-fedcred`). The `agentweaver-agent-host` service account is deliberately federated to a **separate, dedicated identity (`agentweaver-agenthost-identity`, federated credential `agentweaver-agenthost-fedcred`) with no Key Vault roles** (issue #471). The static `agentweaver-secrets` SecretProviderClass defines which Key Vault objects are mounted for the API.
