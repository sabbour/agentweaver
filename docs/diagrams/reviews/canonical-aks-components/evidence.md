# canonical-aks-components: evidence and exclusive claim
...
Target page: `docs/aks-deployment.md`
...
- n0 **Application ingress** — `native:networking`; k8s/base/frontend-deployment.yaml:10; k8s/base/gateway.yaml; k8s/base/gateway-preview.yaml
- n1 **API deployment** — `native:kubernetes`; k8s/base/api-deployment.yaml:13,67-72,415-419
- n2 **MCP deployment** — `native:kubernetes`; k8s/base/mcp-deployment.yaml:10; apps/Agentweaver.Mcp/McpBrokerAuthenticationHandler.cs
- n3 **Worker deployment** — `native:kubernetes`; k8s/base/worker-deployment.yaml:13,67-72,266-270; k8s/base/worker-hpa.yaml:65-66
- n4 **AgentHost pods** — `native:kubernetes`; k8s/base/sandbox-warmpool-agenthost.yaml; apps/Agentweaver.AgentHost/Program.cs:232-285
- n5 **Durable services** — `native:database`; k8s/base/api-deployment.yaml:67-72,415-419; k8s/base/pvc-workspace.yaml; k8s/base/worker-deployment.yaml:266-270

## Every connector's evidence map
- e0: n0 → n1: HTTPS. Evidence: k8s/base/frontend-deployment.yaml:10; k8s/base/gateway.yaml; k8s/base/gateway-preview.yaml; k8s/base/api-deployment.yaml:13,67-72,415-419
- e1: n2 → n1: API tools. Evidence: k8s/base/mcp-deployment.yaml:10; apps/Agentweaver.Mcp/McpBrokerAuthenticationHandler.cs; k8s/base/api-deployment.yaml:13,67-72,415-419
- e2: n1 → n5: persist / mount. Evidence: k8s/base/api-deployment.yaml:13,67-72,415-419; k8s/base/api-deployment.yaml:67-72,415-419; k8s/base/pvc-workspace.yaml; k8s/base/worker-deployment.yaml:266-270
- e3: n3 → n5: persist / mount. Evidence: k8s/base/worker-deployment.yaml:13,67-72,266-270; k8s/base/worker-hpa.yaml:65-66; k8s/base/api-deployment.yaml:67-72,415-419; k8s/base/pvc-workspace.yaml; k8s/base/worker-deployment.yaml:266-270
- e4: n3 → n4: claim + dispatch. Evidence: k8s/base/worker-deployment.yaml:13,67-72,266-270; k8s/base/worker-hpa.yaml:65-66; k8s/base/sandbox-warmpool-agenthost.yaml; apps/Agentweaver.AgentHost/Program.cs:232-285
