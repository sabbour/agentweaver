# experience-a2a-distributed-agents-fig2: pass-02
...
Reading direction, card/boundary separation, title/detail fit, label association and arrow direction reviewed at print size and enlarged. Correction handoff: Outer resume label extends past the printable left page boundary.
...
- e0: Claim a sandbox -> Configure + stream: configure. `apps/Agentweaver.Api/Sandbox/KubernetesSandboxExecutor.cs:663-691`. Trace pending correction passes.
- e1: Configure + stream -> Terminal evidence: turn end. `packages/Agentweaver.AgentRuntime/Workflow/RemoteAgentProxy.cs:408-456`. Trace pending correction passes.
- e2: Configure + stream -> Visible failure: failure. `packages/Agentweaver.AgentRuntime/Workflow/RemoteAgentProxy.cs:408-456`. Trace pending correction passes.
- e3: Visible failure -> Recovery decision: evaluate. `apps/Agentweaver.Api/Coordinator/CoordinatorSteeringDecider.cs:685-743`. Trace pending correction passes.
- e4: Checkpoint wait -> Claim a sandbox: resume. `apps/Agentweaver.Api/Runs/RunWatchLoopService.cs:394-408`. Trace pending correction passes.
