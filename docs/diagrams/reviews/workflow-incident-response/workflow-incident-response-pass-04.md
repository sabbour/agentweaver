...
Stable image: `docs/diagrams/workflow-incident-response.png`; source: `docs/diagrams/src/workflow-incident-response.drawio`.
...
## Complete final arrow trace (7 connectors)

| Connector | Source → target | Relationship | Evidence | Direction / endpoints / route | Result |
|---|---|---|---|---|---|
| edge-01 | triage → mitigate | unconditional advance | `packages/Agentweaver.Squad/Catalog/Resources/workflows/incident_response.yaml:63–64` | source bottom → target top; downward gutter; block arrow at target | clean |
| edge-02 | mitigate → verify | unconditional advance | `packages/Agentweaver.Squad/Catalog/Resources/workflows/incident_response.yaml:66–67` | source bottom → target top; downward gutter; block arrow at target | clean |
| edge-03 | verify → review-gate | unconditional advance | `packages/Agentweaver.Squad/Catalog/Resources/workflows/incident_response.yaml:70–71` | source bottom → target top; downward gutter; block arrow at target | clean |
| edge-04 | review-gate → postmortem | approved | `packages/Agentweaver.Squad/Catalog/Resources/workflows/incident_response.yaml:74–76` | source bottom → target top; downward gutter; block arrow at target | clean |
| edge-05 | review-gate → mitigate | request-changes | `packages/Agentweaver.Squad/Catalog/Resources/workflows/incident_response.yaml:78–80` | source left → dedicated marigold outer rail → target bottom; separate arrowhead; native bridges at crossings | clean |
| edge-06 | review-gate → terminal-declined | declined | `packages/Agentweaver.Squad/Catalog/Resources/workflows/incident_response.yaml:82–84` | source right → distinct outcome rail → intended target side; block arrow; native bridges, no junction implication | clean |
| edge-07 | postmortem → done | unconditional advance | `packages/Agentweaver.Squad/Catalog/Resources/workflows/incident_response.yaml:86–87` | source right → distinct outcome rail → intended target side; block arrow; native bridges, no junction implication | clean |
