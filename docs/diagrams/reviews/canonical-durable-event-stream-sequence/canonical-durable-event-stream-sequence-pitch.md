...
Canonical source: `docs/diagrams/src/canonical-durable-event-stream-sequence.drawio`.
Stable PNG: `docs/diagrams/canonical-durable-event-stream-sequence.png`.
...
- `producer`: custom:agentweaver — Producer.
- `stream`: native:uml — EF stream.
- `store`: native:database — Postgres.
- `sse`: native:uml — SSE.
- `client`: native:uml — Watcher.
