# Full single-agent run · representative lifecycle — pass-05
...
Opened 00-system-overview-fig2-pass-05.png enlarged and 00-system-overview-fig2-pass-05-print.png as an A5/96-dpi screen proof. Correction-only: changed the review condition to no further Rai revision, the arrow label to no revision, and the terminal metadata to RunWatchLoopService:595–681. Opened the newly exported enlarged PNG and A5 proof. All eight arrows were traced again: launch, turn output, review condition, empty diff, approval, unflagged empty result, nonblocked merge result, and workflow output. New subtitle fits; label does not cover the arrowhead; final orientation, overlap and arrow defect counts are zero. No other semantic or layout changes.
...
- `rai-to-review` | rai | review | no revision | apps/Agentweaver.Api/Workflows/RunWorkflowGraphBinder.cs:372-379 | source→target block arrowhead | endpoints on intended card | orthogonal route checked in opened PNG | clean
