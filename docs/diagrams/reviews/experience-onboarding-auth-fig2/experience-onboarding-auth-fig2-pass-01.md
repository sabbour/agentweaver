# experience-onboarding-auth-fig2: pass-01
...
Metric: visible-semantic-canonical-xml-v1. Baseline 1041; result 17903; ratio 17.197887x.
...
Reading direction, card/boundary separation, title/detail fit, label association and arrow direction reviewed at print size and enlarged. Correction handoff: The tool-bearer label obscures the arrowhead.
...
- e0: MCP client -> Browser consent: open. `apps/Agentweaver.Api/Endpoints/OAuthAuthorizationServerEndpoints.cs:87-138`. Trace pending correction passes.
- e1: Browser consent -> OpenIddict: allow. `apps/Agentweaver.Api/Endpoints/OAuthAuthorizationServerEndpoints.cs:148-188`. Trace pending correction passes.
- e2: OpenIddict -> Token exchange: code grant. `apps/Agentweaver.Api/Endpoints/OAuthAuthorizationServerEndpoints.cs:31-76`. Trace pending correction passes.
- e3: Token exchange -> MCP boundary: tool bearer. `apps/Agentweaver.Mcp/McpBrokerAuthenticationHandler.cs:40-87`. Trace pending correction passes.
- e4: MCP boundary -> Authorized API: forward. `apps/Agentweaver.Mcp/AgentweaverApiClient.cs:359-382`. Trace pending correction passes.
