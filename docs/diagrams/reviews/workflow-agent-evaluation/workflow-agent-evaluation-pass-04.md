...
Stable image: `docs/diagrams/workflow-agent-evaluation.png`; source: `docs/diagrams/src/workflow-agent-evaluation.drawio`.
...
## Complete final arrow trace (8 connectors)

| Connector | Source → target | Relationship | Evidence | Direction / endpoints / route | Result |
|---|---|---|---|---|---|
| edge-01 | eval-setup → eval-run | unconditional advance | `packages/Agentweaver.Squad/Catalog/Resources/workflows/agent_evaluation.yaml:63–64` | source bottom → target top; downward gutter; block arrow at target | clean |
| edge-02 | eval-run → eval-collect | unconditional advance | `packages/Agentweaver.Squad/Catalog/Resources/workflows/agent_evaluation.yaml:66–67` | source bottom → target top; downward gutter; block arrow at target | clean |
| edge-03 | eval-collect → safety-gate | unconditional advance | `packages/Agentweaver.Squad/Catalog/Resources/workflows/agent_evaluation.yaml:69–70` | source bottom → target top; downward gutter; block arrow at target | clean |
| edge-04 | safety-gate → eval-setup | revise | `packages/Agentweaver.Squad/Catalog/Resources/workflows/agent_evaluation.yaml:73–75` | source left → dedicated marigold outer rail → target bottom; separate arrowhead; native bridges at crossings | clean |
| edge-05 | safety-gate → terminal-safety-failed | safety-failed | `packages/Agentweaver.Squad/Catalog/Resources/workflows/agent_evaluation.yaml:77–79` | source right → distinct outcome rail → intended target side; block arrow; native bridges, no junction implication | clean |
| edge-06 | safety-gate → done | no-changes | `packages/Agentweaver.Squad/Catalog/Resources/workflows/agent_evaluation.yaml:81–83` | source right → distinct outcome rail → intended target side; block arrow; native bridges, no junction implication | clean |
| edge-07 | safety-gate → report | review | `packages/Agentweaver.Squad/Catalog/Resources/workflows/agent_evaluation.yaml:85–87` | source bottom → target top; downward gutter; block arrow at target | clean |
| edge-08 | report → done | unconditional advance | `packages/Agentweaver.Squad/Catalog/Resources/workflows/agent_evaluation.yaml:89–90` | source right → distinct outcome rail → intended target side; block arrow; native bridges, no junction implication | clean |
