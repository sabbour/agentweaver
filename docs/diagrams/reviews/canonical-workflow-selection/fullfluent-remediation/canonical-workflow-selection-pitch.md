# canonical-workflow-selection — pitch
...
Both populated responsibility cards, native process/decision glyphs, colored pill badges and full Fluent hierarchy are visible and legible. No text clipping, card overlap or off-page content observed at print size or enlarged view. The central arrow is an overview relationship, not an exhaustive execution branch list; pass 1 expands the source-backed topology.
...
The populated responsibility model is deliberately compact, not a full execution graph. Pass 1 must expand the authoritative authored graph, retaining every declared branch. The old black/white pitch is superseded, not retrospectively certified. Do not add catalog merge/PR/Scribe, fan-out/fan-in, or inferred git push.
