...
For AKS production, run `pnpm run infra:deploy` before the first release
deployment; it provisions the signing key together with the required cluster
identity and secrets. Use `npm run infra:deploy` if npm is your selected
package runner. The installer's `--skip-oauth-key` flag is only safe when that
