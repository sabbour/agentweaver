...
- **Milestone procedure** (same as the existing Release Pipeline below, just cadence-gated):
  ...
  3. Build and push with `npm run azure:devtest` (or cut a semver release with `npm run azure:release`), then verify with `npm run azure:verify`.
...
1. Cut the `v0.9.49-rc1` milestone: batch-commit the full pending set (#227/#309, #308, #306, #224, #216, #278, #303, #266-retry — resource-sizing #307 already committed separately at `fcc338bf`), bump `VERSION`, run `npm run azure:devtest` (or `npm run azure:release` to cut a tagged release).
...
Bump `VERSION` → `npm run azure:release` (or `npm run azure:devtest` for an image-only redeploy) → `npm run azure:verify`.
