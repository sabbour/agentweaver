...
- **Milestone procedure** (same as the existing Release Pipeline below, just cadence-gated):
  ...
  3. Build and push with `pnpm run release:images` → deploy and verify with `pnpm run release:deploy`.
...
1. Cut the `v0.9.49-rc1` milestone: batch-commit the full pending set (#227/#309, #308, #306, #224, #216, #278, #303, #266-retry — resource-sizing #307 already committed separately at `fcc338bf`), bump `VERSION`, run `pnpm run release:images`, then run `pnpm run release:deploy`.
...
Bump `VERSION` → `pnpm run release:images` → `pnpm run release:deploy`.
