...
**1. What it is.** A real custom agent, **invokable directly by Squad** (agent-to-agent
call) or run manually by Ahmed — not a script, and not only a skill invocation. When run
for a full pass, it:
...
- (d) **synthesizes a human-readable narrative + evidence bundle** — cross-surface
  root-cause attribution and recommendations — the part a deterministic script can't do
  well but an LLM agent can.

**2. Pure test executor + evidence producer — Harness takes no GitHub actions.** The
Harness agent's only job is to **run the requested test scenario and produce structured
evidence/observability**: verdict JSON, screenshots, AppInsights/kubectl correlation, and
`run_id`/`trace_id`. It returns that bundle to **whoever called it** (Squad or Ahmed) and
stops there. Harness does **not** decide issue lifecycle and **never files, comments on,
labels, triages, or closes GitHub issues at all**. All GitHub issue authority — filing,
