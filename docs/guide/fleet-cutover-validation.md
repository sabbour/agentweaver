---
title: GitHub connections cutover validation
---
...
| F2 — storage and migration | #940, #951 | On a production-like database copy, apply the forward migrations once, restart all API replicas, and verify durable GitHub connections records, constraints, project cascades, append-only audit records, and immutable snapshots. Re-run migration discovery without applying a second change. Exercise the documented SQLite-to-PostgreSQL transfer failure injection and prove no partial GitHub connections project records remain. | Migration failure, provider-schema drift, partial transfer, duplicate active binding/claim, persisted credential material, or a `Down`/rollback path that restores legacy credential storage. |
