...
Each project has a `SandboxPolicy` record stored in SQLite, keyed by `repository_path`. When no record exists for a repository, `SandboxPolicy.Default(repositoryPath)` applies.
