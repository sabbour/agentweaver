...
| `coordinator.assembly_blocked` | When assembly stops with NO partial work — an ineligible subtask set, or a conflict building the integration branch | `workPlanId`, `reason`, and either `ineligibleSubtaskIds` / `ineligibleSubtasks` or `conflictingBranch` / `conflictingFiles` |
...
2. **Eligibility gate (no partial assembly)** — every subtask must be assembly-eligible (`assemble_ready`, or `completed` with no changes). If any is failed / rai_flagged / pending / blocked, or merging the eligible child branches into the integration branch conflicts, the pipeline emits `coordinator.assembly_blocked` and STOPS — no RAI, no merge. The ineligible-subtasks path includes `ineligibleSubtaskIds` plus enriched `ineligibleSubtasks`; the conflict path includes `conflictingBranch` and `conflictingFiles`.
