...
| `tool.auto_approved` | When the per-run safe-tool option auto-grants a repository-defined safe tool instead of waiting for a human; audit-only (the tool then runs) | `requestId`, `toolName`, `url` (optional) |
...
| `run.approval_policy_selected` | When a coordinator run persists its immutable launch approval policy | `autoApproveTools`, `autopilot`, `source` (`direct`, `backlog_pickup`, or `retry`), `safeTools` |
