...
After every child subtask finishes, the coordinator runs ONE collective assembly: it builds a single integration branch (all eligible child branches merged in dependency order off the originating branch), then runs the selected workflow's assembly gates in happy-path traversal order. For software workflows that path places RAI before Build & Test and human review. Build & Test is an automated platform gate over a detached integration-branch worktree; in `Sandbox:AgentExecutionMode=pod-per-run`, it binds a dedicated AgentHost sandbox pod to the coordinator run id and configures that pod to use the detached worktree as its working directory before running the turn (`apps/Agentweaver.Api/Coordinator/CollectiveAssemblyPipeline.cs:155`, `apps/Agentweaver.Api/Sandbox/KubernetesSandboxExecutor.cs:300`). That same run-bound pod is the target for any `start_preview` URL during review. Before Build & Test approval is applied, the coordinator records preview applicability and requires a final preview outcome: `sandbox.preview_ready`, `sandbox.preview_failed`, or `sandbox.preview_skipped_not_applicable`. If none exists, it emits `sandbox.preview_failed` with `reason: "preview_outcome_missing"` and still lets human review proceed with preview unavailable (`apps/Agentweaver.Api/Coordinator/CoordinatorAssemblyService.cs:688`, `:708`, `:2031`). Human review is the only gate that accepts `POST /api/runs/{coordinatorRunId}/assembly/review`. On approve it merges, runs the collective scribe, and completes; when Build & Test or Rubberduck requests changes, it retains the assembly Build & Test pod and detached worktree, re-dispatches the inferred children, and reuses the existing run-bound claim on the next assembly pass instead of cold-launching another pod (`apps/Agentweaver.Api/Coordinator/CoordinatorAssemblyService.cs:1536`, `apps/Agentweaver.Api/Sandbox/KubernetesSandboxExecutor.cs:310`). Non-automated request-changes and terminal paths still clean up the assembly resources. The web run page projects a request-changes event into a visible revision state: **Revising after {Gate} feedback** on the coordinator chip, **Changes requested — revising** on affected subtasks, and a `Request changes from {Gate}` timeline entry with feedback (`apps/web/src/pages/CoordinatorRunPage.tsx:713`, `:2703`; `apps/web/src/components/AgentSessionPanel.tsx:1599`). Sandbox infrastructure failures are classified separately as `build_test_infra_*`: retryable cases park as `assembly_blocked`, and non-retryable configuration errors fail assembly. Their event payloads include `detail`, `exceptionMessage`, `innerExceptionMessage`, `innerExceptionType`, and `infrastructureReason`, so `/api/runs/{id}/events` shows the underlying AgentHost launch or transport failure instead of only the generic reason (`apps/Agentweaver.Api/Coordinator/CoordinatorAssemblyService.cs:1590`, `:1604`). The review POST is replica-safe: a non-owner API replica can persist a deferred decision while the work plan is durably `in_review`, and the owner pipeline consumes it at most once. The preview event sequence is documented in [Live-preview provisioning](./live-preview-provisioning.md), and the full stream sequence is documented in the [events reference](./events.md).
