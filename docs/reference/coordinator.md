...
- **auto-approve-tools** auto-grants only repository-defined safe tools (currently `web_fetch`) at the human-in-the-loop gate, logging each as `tool.auto_approved { requestId, toolName, url? }`. It never covers preview (`start_preview`), arbitrary shell execution, destructive or privileged actions, secrets, or tools outside the explicit safe list. Existing scoped approval policy can still permit a separately eligible action through its normal gate; the per-run flag does not broaden that policy.
