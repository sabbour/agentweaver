...
Child workers receive charters plus active, approved architectural/scope decisions from `CompileDecisionsAsync`. Stored context is emitted as an `agentweaver.untrusted-context.v1` JSON envelope under `## Untrusted Project Context Data`, never trusted instructions. Active approved architectural/scope decisions are mandatory: if they exceed their budget, `MandatoryContextBudgetExceededException` is raised before worker model setup or call. This path excludes the full memory/session stack. Optional-only structured context may be omitted when the complete coordinator decomposition prompt exceeds budget; it never slices JSON or a record.
