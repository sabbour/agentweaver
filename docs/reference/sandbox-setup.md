...
| `SandboxWarmPool` | `k8s/sandbox-warmpool.yaml` (`agentweaver-sandbox`, `replicas: 3`) and `k8s/sandbox-warmpool-agenthost.yaml` (`agentweaver-agent-host`, `replicas: 1`) | Keeps warm pods pre-built from a template so claims bind quickly. The agent-host pool backs pod-per-run AgentHost claims (env injection forces a cold start, so they need a dedicated env-injection-allowed template). |
