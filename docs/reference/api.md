...
Run control state is durable. Tool approval requests, run-scoped/always allow policies, child-to-parent approval inheritance, `auto-approve`, and `autopilot` are replayed from persisted run events. That means approval and toggle requests may land on any API replica and still be observed by the worker that owns the run.
