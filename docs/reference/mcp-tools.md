...
| `github_repo_app_connect` | Begin the current human's Repo App authorization. Returns an opaque transaction ID, a browser URL, and expiry. Open browser_url in a browser, then poll github_repo_app_authorization_status. No credential, OAuth state, or callback cookie is returned. |
...
| `project_copilot_app_connect` | Begin an Owner-authorized, project-bound Copilot App connection. Returns an opaque transaction ID, browser URL, and expiry. Open browser_url, then poll project_copilot_app_authorization_status. No credential, OAuth state, callback cookie, repository, installation, or permission data is returned. |
...
| `skill_marketplace_browse` | Browse or search a curated marketplace (paginated) without changing the project catalog. |
...
| `skill_marketplace_source_add` | Add a project-scoped skill marketplace source by public GitHub repo URL or owner/repo. Subpath is optional (auto-detected when omitted). The source is then browsable/importable by its name. |
| `skill_marketplace_source_remove` | Remove a project-scoped marketplace source by name. Administrator-curated config marketplaces cannot be removed. |
| `skill_marketplace_sources_list` | List all skill marketplaces available to a project: administrator-curated config marketplaces plus the project's own URL-added sources. |
