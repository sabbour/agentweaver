...
_Pending. The following open questions are flagged inline as [NEEDS CLARIFICATION] and should be resolved via a `/speckit.clarify` pass before planning:_

- **C1 — Section placement of Memories**: Memories (Feature 006, `MemoriesPage`) has no Squadboard equivalent. Does it belong under SQUAD (alongside Agents) or OPERATIONS? See FR-009.
- **C2 — Diagnostics / Heartbeat backing**: Agentweaver has a `CoordinatorHeartbeatService` background service (`apps/Agentweaver.Api/Coordinator/CoordinatorHeartbeatService.cs`) and only a plain-text root endpoint (`GET /` returns "Agentweaver API", `apps/Agentweaver.Api/Endpoints/RunEndpoints.cs:30`). There is **no structured health, version, diagnostics, or heartbeat-status API endpoint** today. Does this iteration build those endpoints, or show the SYSTEM nav items as disabled/coming-soon until a later iteration? See FR-016, FR-017.
- **C3 — Health/status dot semantics**: What does the green status dot in the top bar represent for Agentweaver (API reachable? coordinator heartbeat alive within interval? both)? See FR-013.
- **C4 — Shell scope for deep run pages**: Do the deep run / execution routes (`WatchPage`, `WorkflowRunPage`, `CoordinatorRunPage`, `CastingWizardPage`) render inside the shell with the project left-nav, or as full-screen flows without it? See FR-006, FR-018.
- **C5 — Project switcher and creation**: Does the top-bar project switcher only switch among existing projects, or does it also offer inline "Add Project" / "Create from template" affordances (the Projects-landing redesign is otherwise deferred)? See FR-011.
...
- **SYSTEM destinations without a backend**: If Diagnostics or Heartbeat has no backing API endpoint in the current build, the corresponding item MUST be shown as unavailable/disabled rather than navigating to a page that fabricates data (Principle VII). [NEEDS CLARIFICATION: see C2 — are these endpoints built in this iteration?]
...
- **FR-006**: The shell MUST render for all project-scoped destinations, including when reached by direct deep link, so navigation is never lost on a project page. [NEEDS CLARIFICATION: see C4 — whether deep run/execution pages (`WatchPage`, `WorkflowRunPage`, `CoordinatorRunPage`, `CastingWizardPage`) render inside the shell or as full-screen flows.]
...
- **FR-009**: The **Memories** item MUST route to the existing Memories page (`MemoriesPage`, `/projects/:projectId/memories`). [NEEDS CLARIFICATION: see C1 — whether Memories sits under SQUAD or OPERATIONS.]
...
- **FR-011**: The top bar MUST include a **project switcher** that displays the current project's name and lets the user change the active project in place, without first returning to the project gallery. [NEEDS CLARIFICATION: see C5 — whether the switcher also offers inline project creation.]
...
- **FR-013**: The top bar MUST display a health/status indicator (status dot) reflecting real system state served by an API endpoint, not a hardcoded value. [NEEDS CLARIFICATION: see C3 — the precise meaning of the indicator for Agentweaver.]
...
- **FR-016**: The sidebar MUST include a **Diagnostics** destination in the SYSTEM section. Any data it surfaces MUST come from a real API endpoint (Principle VII). [NEEDS CLARIFICATION: see C2 — whether the diagnostics endpoint/page is built in this iteration or the item is shown as unavailable until a later one.]
- **FR-017**: The sidebar MUST include a **Heartbeat** destination in the SYSTEM section that surfaces the state of the existing coordinator heartbeat (`CoordinatorHeartbeatService`, `apps/Agentweaver.Api/Coordinator/CoordinatorHeartbeatService.cs`). The heartbeat state MUST be exposed by a real API endpoint, since none exists today (the service runs server-side with no status surface). [NEEDS CLARIFICATION: see C2.]
- **FR-018**: If a SYSTEM destination has no backing endpoint in the current build, its sidebar item MUST be shown as unavailable/disabled rather than navigating to a page that fabricates data.
...
- **System Status Indicator**: The top-bar health/status dot reflecting real, API-served system state (precise meaning pending clarification C3).
- **Heartbeat Status**: The state of the existing `CoordinatorHeartbeatService` surfaced by the Heartbeat destination; requires a new read-only API endpoint to expose. Referenced from Feature 008/009; not redefined here.
...
- Agentweaver has a running coordinator heartbeat (`CoordinatorHeartbeatService`) but **no** structured health/diagnostics/heartbeat API endpoint today (only `GET /` returning the plain string "Agentweaver API", `apps/Agentweaver.Api/Endpoints/RunEndpoints.cs:30`). Surfacing Diagnostics and Heartbeat therefore implies new read-only API endpoints; whether they are built this iteration is pending clarification (C2).
