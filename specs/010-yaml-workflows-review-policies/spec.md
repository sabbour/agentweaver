...
- Q: Are workflow files hot-reloaded on change, or loaded once at project/run start? → A: Loaded once (load-on-start / on explicit sync); NOT file-watch and NOT per-heartbeat. A Workflows management page in the web app provides a "Sync" button (modeled on the existing Team page sync affordance) that re-reads `.scaffolders/workflows/` and refreshes the loaded set. In-flight runs always complete on the definition they started with. Listing and syncing workflows are also available via the MCP server (parity).
- Q: How does a project choose its effective workflow when multiple valid workflows exist? → A: Each project has a DEFAULT workflow; a workflow can be OVERRIDDEN PER TASK (per work item). When a task is in Backlog/Ready the user may select a non-default workflow for that specific task; absent an override the project default applies. Trigger type is a property of the workflow (some run on heartbeat schedule, some on events); workflow choice is NOT bound to trigger.
- Q: Which events are valid event triggers and what is their payload contract? → A: For this iteration the ONLY supported event trigger is "task added to Ready". The schema may remain extensible, but only "task added to Ready" is implemented/valid now; all other event types are out of scope for this iteration.
- Q: Where do Review Policy steps inject when the workflow declares no explicit review point? → A: At a single implicit pre-merge review point. The DEFAULT review steps applied there are Rubber-duck review and RAI review (both run pre-merge by default). Human-review is NOT injected by default; it applies only when explicitly declared/configured for the project.
- Q: Where is a Review Policy authored/stored and how does a project bind to it? → A: Both: a Review Policy may be authored as a file under `.scaffolders/` (e.g. `.scaffolders/review-policies/`) AND/OR stored as an API project setting. A project binds its policy BY NAME.
...
- **Changing a workflow definition while a run is in flight**: An in-flight run MUST complete on the workflow definition it started with; redefinition MUST NOT mutate a running graph mid-execution. Workflow files are loaded once (load-on-start) and refreshed only on an explicit Sync action (not file-watch, not per-heartbeat), so a Sync that picks up an edited definition affects only runs started after the Sync.
- **Multiple workflows defined**: When `.scaffolders/workflows/` contains more than one valid workflow, the project's *effective* workflow for a given work item is its per-project **default workflow**, unless that task carries a **per-task workflow override**, in which case the override applies. Workflow choice is independent of trigger type (trigger is a property of each workflow).
...
- **Event trigger semantics**: For this iteration the only supported event trigger is **"task added to Ready"**; a workflow declaring any other event type MUST fail validation. The schema MAY remain extensible for future events, but only "task added to Ready" is valid/implemented now.
...
- **FR-006**: Workflow definitions MUST be loaded once (load-on-start) and refreshed only on an explicit **Sync** action — NOT via file-watch and NOT per coordinator heartbeat. An in-flight run MUST complete on the workflow definition it started with; a Sync that picks up an edited definition MUST affect only runs started after the Sync.
...
- **FR-039**: The Web UI MUST provide a **Workflows management page** that lists the project's discovered workflows with their validation status and provides a **Sync** action that re-reads `.scaffolders/workflows/` and refreshes the loaded set. This page MUST follow the existing Team page pattern (which exposes a sync affordance).
- **FR-040**: Listing workflows (with validation status) and triggering a workflow **Sync** MUST be reachable identically from the MCP server and the Web UI, with the discovery, validation, and refresh logic performed server-side and no business logic in either client (Principles III, IV).
...
- **FR-022**: The schema MUST support an **event** trigger. For this iteration the only valid event is **"task added to Ready"**: a run MUST begin when a task enters the project's Ready bucket (Feature 009) and not otherwise. A workflow declaring any other event type MUST fail validation; the schema MAY remain extensible for future events.
...
- **FR-041**: Each project MUST have a **default workflow** setting that selects, by id/name, which available workflow applies to its work items absent an override.
- **FR-042**: A work item (task) MUST be able to carry a **per-task workflow override** selecting a non-default available workflow for that specific task while it is in Backlog or Ready; absent an override, the project default (FR-041) applies. Selecting/changing the override MUST be reachable identically from the MCP server and the Web UI.
...
- **FR-026**: The supported review steps MUST include an **RAI step**, a **Rubber-duck step**, and a **Human-review step**, mapping respectively onto the RAI verdict gate (`RaiTurnExecutor`), the request-changes-to-producer loop, and the human-approval `RequestPort` gate. RAI and Rubber-duck steps run pre-merge by default (FR-032); the Human-review step is opt-in and applies only when explicitly declared/configured for the project.
...
- **FR-028**: The review steps in a project's Review Policy MUST inject into the project's workflow runs at the defined review point(s). When the effective workflow declares no explicit review point, the steps MUST inject at a **single implicit pre-merge review point** rather than being dropped. By default the implicit pre-merge point runs the **Rubber-duck step and the RAI step**; the **Human-review step** is NOT injected by default and applies only when explicitly declared/configured for the project.
...
- **FR-032**: A project with no explicitly configured Review Policy MUST receive a safe **default policy** consisting of the **Rubber-duck step and the RAI step at the implicit pre-merge review point** (FR-028). This default MUST preserve, at minimum, RAI content-safety failure (FR-030); a Human-review gate for irreversible actions applies when explicitly configured (FR-029) and is otherwise governed by the runtime's mandatory human-approval-for-irreversible-action enforcement (FR-034).
- **FR-033**: A Review Policy MUST be selectable by a project **by name**, and MUST be authorable BOTH as a file under `.scaffolders/` (e.g. `.scaffolders/review-policies/`) AND/OR as an API-stored project setting. The system MUST resolve a project's named policy from either source; a project binds to its policy by name rather than by inlining the policy definition.
...
- **Trigger**: The declared start condition of a workflow — **manual** (explicit start), **heartbeat schedule** (coordinator heartbeat pickup → `CoordinatorHeartbeatService`/`CoordinatorPickupService`), or **event** (this iteration: only "task added to Ready", per Feature 009). Trigger is a property of the workflow and does not determine which workflow a project/task selects.
...
- **Review Policy**: A first-class, named, per-project configuration composed of **review steps** — **RAI step** (`RaiTurnExecutor`), **Rubber-duck step** (request-changes loop), **Human-review step** (`RequestPort` human-approval gate). Authorable as a `.scaffolders/review-policies/` file and/or an API project setting; a project binds to it **by name**. Injected at the project's review points, including a single implicit pre-merge point when the workflow declares none, and the collective-assembly review point. The default policy (when none is configured) is Rubber-duck + RAI pre-merge; Human-review is opt-in.
- **Project Default Workflow**: A per-project setting naming which available workflow applies to the project's work items absent a per-task override.
- **Per-Task Workflow Override**: An optional selection on a work item (task) naming a non-default available workflow for that specific task while it is in Backlog or Ready; absent it, the project default applies.
- **Effective Workflow (per work item)**: The resolved Workflow Definition a given task actually runs — the per-task override if present, otherwise the project default — drawn from the available workflows (template-provided and `.scaffolders/workflows/`), with the project's Review Policy injected. The single source of truth for run execution and for derived views (e.g., Feature 009's board columns).
- **Workflows Management Page / Sync**: A Web UI surface (modeled on the existing Team page) that lists the project's discovered workflows with validation status and exposes a **Sync** action to re-read `.scaffolders/workflows/`; the same list/sync capability is exposed via the MCP server.
...
All open questions were resolved in the Clarifications session (2026-06-22). The decisions are encoded in the corresponding requirements:

1. **Load-on-start vs. hot-reload** → RESOLVED (FR-006, FR-039, FR-040): Load-on-start with an explicit Sync action surfaced via a Workflows management page (Web) and the MCP server; no file-watch or per-heartbeat reload; in-flight runs unaffected by a later Sync.
2. **Effective-workflow selection with multiple files** → RESOLVED (FR-041, FR-042): Per-project default workflow plus optional per-task override; trigger type is a property of each workflow and does not bind workflow selection.
3. **Event-trigger catalog** → RESOLVED (FR-022): Only "task added to Ready" is valid this iteration; the schema may remain extensible for future events.
4. **Review-step injection points** → RESOLVED (FR-028, FR-032): Single implicit pre-merge review point; default steps are Rubber-duck and RAI; Human-review is opt-in.
5. **Review Policy storage/binding** → RESOLVED (FR-033): Authorable as a `.scaffolders/review-policies/` file and/or an API project setting; bound to a project by name.
