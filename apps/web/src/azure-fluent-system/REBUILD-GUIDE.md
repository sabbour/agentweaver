...
`azure-fluent-system` barrel; style only with its tokens; never introduce raw hex, px colors, or a
...
> You are rebuilding the Agentweaver web app on the in-repo `azure-fluent-system` design library.
> Read, in order: `azure-fluent-system/DESIGN.md`, `README.md`, `COVERAGE.md`,
> `catalog/PATTERNS.md`, `patterns.tsx`, and the relevant `examples/*.example.tsx`. Rules: import
> only from the `azure-fluent-system` barrel; style only with `tokens.css` tokens (no raw
