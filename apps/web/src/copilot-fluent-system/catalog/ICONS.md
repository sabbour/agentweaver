...
Checked-in icon inventory for `apps/web/src/copilot-fluent-system`, sourced from IconCloud Azure Icons exports and local icon helpers. This catalog records vendored Azure assets, local alias strategies, and icon examples without dumping manifest JSON or SVG payloads.
...
| Category/name manifest keys | Local key convention for manifest-backed Azure assets | local-alias | Not recorded | `createIconCloudRegistryFromManifest()` in `icons.tsx`; sample wiring in `examples/icon-registry.example.tsx` | Partial | Documents the stable `Category/Icon name` registry key shape used by downstream consumers when they build registries from the vendored manifest. |
