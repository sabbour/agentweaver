    ...
    public IEnumerable<AIFunction> BuildTools(SandboxToolContext context)
    {
        yield return AIFunctionFactory.Create(
            async (
                ...
                [Description("Working directory for the command. Defaults to the current run worktree. Accepts either a sandbox-relative path or an absolute path that resolves to the sandbox root or one of its subdirectories.")] string? cwd = null,
...
internal sealed class PreviewRunner : BackgroundService, IPreviewRunner
{
        ...
        CancellationToken ct = default)
    {
        ...
        var fullCwd = string.IsNullOrWhiteSpace(sandboxRoot)
            ...
            : SandboxPathValidator.ValidateRelativeOrAbsoluteContained(cwd, sandboxRoot);
