...
export function renderCaptureScript(plan) {
  ...
  lines.push(
    ...
    '      .locator(\'[data-testid="session-approval-gate"], [data-testid="assistant-approval-gate"], [role="alert"]\')',
    '      .filter({ hasText: \'Tool Approval Required\' })',
    '      .getByRole(\'button\', { name: /^(Allow once|Approve)$/ })',
    '      .first();',
    '    while (watcherActive) {',
    '      try {',
    '        if (await approvalButton.isVisible().catch(() => false)) {',
    '          await pause(approvalWatcherGraceMs);',
