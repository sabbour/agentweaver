...
export function printReport(finding) {
  ...
  if (approvals.length) {
    ...
    for (const a of approvals) {
      ...
      const mark = d === 'approve' ? `${GREEN}approve${RESET}` : d === 'deny' ? `${RED}deny${RESET}` : d === 'request-changes' ? `${YELLOW}request-changes${RESET}` : `${YELLOW}defer${RESET}`;
      ...
      if (d === 'request-changes' && a.feedback) {
        console.log(`    ${DIM}${a.feedback.summary}${RESET}`);
      }
    }
    console.log('');
  }
