...
test('driveScenario records request-changes feedback once without resolving the gate as approve or deny', async () => {
  const client = scriptedClient();
  let judgeCalls = 0;
  const judge = async () => {
    judgeCalls += 1;
    return {
      ...
      reason: 'the proposed fetch needs revision',
  ...
  const result = await driveScenario(client, scenario, persona, {
    timeoutMs: 30, pollMs: 1, keep: true, driveApprovals: true, judge,
  });

  assert.equal(judgeCalls, 1, 'the feedback decision is handled once, not retried every poll');
  assert.equal(client.calls.some((c) => c.path.includes('/tool-approvals') || c.path.includes('/tool-denials')), false);
  const [decision] = result.evidence.approvalDecisions;
  assert.equal(decision.executed, false);
  assert.equal(decision.requiresChanges, true);
  assert.deepEqual(decision.feedback, { summary: 'Constrain the request.', requestedChanges: ['Use an allowed host.'] });
});
