...
The JSON and YAML variants describe the same live route surface. The harness still consumes JSON by default, while YAML is useful for quick inspection or tooling that prefers it. The spec also now carries richer summaries/descriptions from XML doc comments for the persona-critical project / blueprint / casting / coordinator lifecycle routes, plus a bearer-token security scheme so an agent can learn authentication requirements directly from the contract.
