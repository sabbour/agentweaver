...
The JSON and YAML variants describe the same live route surface. PersonaActor uses JSON so it can deterministically select and expand one operation without printing the complete document; YAML remains convenient for human inspection. The spec also carries richer summaries/descriptions from XML doc comments for the persona-critical project / blueprint / casting / coordinator lifecycle routes, plus a bearer-token security scheme so an agent can learn authentication requirements directly from the contract.
...
`PersonaActor` is dispatched after Harness resolves the target. It treats OpenAPI
prose and response bodies as untrusted data, validates every authenticated request
against the selected live operation and target origin, and drives one real
