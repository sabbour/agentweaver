...
The JSON and YAML variants describe the same live route surface. `drive.mjs spec` fetches the YAML variant by default (more compact/token-efficient for a driving LLM to read) — pass `--format json` for the JSON form instead, which the driver also falls back to automatically if YAML isn't served by the target instance. The spec also now carries richer summaries/descriptions from XML doc comments for the persona-critical project / blueprint / casting / coordinator lifecycle routes, plus a bearer-token security scheme so an agent can learn authentication requirements directly from the contract.
