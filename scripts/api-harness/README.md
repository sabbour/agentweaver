...
**Persona-behavior scenarios (Priya, Jordan, ...) are no longer fixed scripts.** There is no per-persona `scenarios/*.mjs` step sequence to run — a driving LLM (Harness itself, or a sub-agent it dispatches) reads the relevant persona-brief + surface adapter, discovers the live API contract via `drive.mjs spec`, and decides every next action live via `drive.mjs call`, including if/when to push back with a revision. See `drive.mjs`'s header comment and `.github/agents/harness.agent.md` for the full model. `run-persona.mjs` remains only for `generated-artifacts-seam` — a deterministic structural conformance check with no persona/pushback dimension.
...
# ...the driving LLM continues issuing `call`s guided by the persona brief + spec...
