...
// access (delegated subnet + private DNS zone + VNet link), and stores the
// admin credentials in a K8s Secret ('agentweaver-postgres'). Idempotent at
// every step. The generated admin password is registered with
// lib/secret.mjs's redaction registry immediately and NEVER logged.
//
// cfg is the resolved variables.mjs output: RESOURCE_GROUP, CLUSTER_NAME,
// LOCATION, NAMESPACE. Optional PG_* overrides (server name, DB name, admin
// user, version, SKU, storage, HA mode, backup days, subnet name/prefix, DNS
// zone/link name) and AKS_VNET_NAME/AKS_MC_RG (auto-detected if unset).
...
export async function run(cfg, opts = {}) {
  ...
  let AKS_VNET_NAME = cfg.AKS_VNET_NAME;
  if (!AKS_VNET_NAME) {
    const result = await exec.capture(
      "az",
  ...
  const AKS_VNET_ID = `/subscriptions/${SUBSCRIPTION_ID}/resourceGroups/${AKS_MC_RG}/providers/Microsoft.Network/virtualNetworks/${AKS_VNET_NAME}`;
  const PG_DNS_ZONE_ID = `/subscriptions/${SUBSCRIPTION_ID}/resourceGroups/${cfg.RESOURCE_GROUP}/providers/Microsoft.Network/privateDnsZones/${PG_DNS_ZONE}`;
  const PG_FQDN = `${PG_SERVER_NAME}.postgres.database.azure.com`;
