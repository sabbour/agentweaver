...
## start_preview callers need an independent completion deadline
...
Cached SSO completes the recorder sign-in with no interactive prompt. The recorder copies the literal Chrome Default profile to a disposable dir and clicks Agentweaver own sign-in button. The ONLY real prerequisite for manual `signin` is that all Chrome processes are closed first - otherwise it fails with "Google Chrome is still running", which is a profile lock, not an auth wall. Close Chrome gracefully via CloseMainWindow() so tabs are preserved, then run: node scripts/demo-recording/cli.mjs signin --base-url <base>. Tokens last about 1.5 hours; the recorder now decodes `exp`, refreshes with a 15-minute margin before long captures, and `status` reports real remaining lifetime. An expired or near-expiry access token is a reason to run `open`, `start`, or authenticated `capture` so cached SSO can refresh; manual `signin` is needed only when the SSO session itself no longer returns automatically. This is recorded because an agent previously fabricated a "refreshing is human-only" rule, committed it to a SKILL.md, and then cited its own writing back as proof the task was impossible.
