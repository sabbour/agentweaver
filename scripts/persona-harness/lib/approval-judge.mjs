...
// command) — should the driver approve, deny, or defer it? Exactly like judge.mjs,
...
/** The three decisions the judge may return. */
export const APPROVAL_DECISIONS = Object.freeze(['approve', 'deny', 'defer']);
 ...
 * @param {any} raw
 * @returns {{decision:'approve'|'deny'|'defer', scope:string, reason:string}}
 */
...
export function buildApprovalDecisionPrompt(gate, context = {}) {
  ...
  const decisionTemplate = {
    ...
    scope: 'once | run | tool | always   (only meaningful for a tool approve; ignored for shell/deny/defer)',
  ...
  return [
    ...
    'proceed — **that is your job.** Return one decision: approve, deny, or defer.',
    ...
    fence(gate?.evidenceEvent ?? null),
    '',
    '---',
    '## Recent run events (context around the gate)',
    '',
    recentEvents.length ? fence(recentEvents) : '(none captured)',
    '',
    '---',
    '## Recent persona turns (intent + composition leading up to the gate)',
    '',
    recentTurns.length ? fence(recentTurns) : '(none captured)',
    '',
    '---',
    '## Run metadata',
    '',
 ...
 * returns null for `defer` (nothing to execute).
 * @param {object} gate
...
export function planApprovalCall(gate, decision, runId) {
  if (!gate || !decision || decision.decision === 'defer') return null;
 ...
 * `deferred` marker) for the audit trail. The driver executes EXACTLY the decision;
 * it applies no policy of its own.
...
export async function executeApprovalDecision(client, runId, gate, decision) {
  ...
  if (!plan) {
    return { gate, decision, apiCall: null, executed: false };
  }
  const apiCall = await client.call(plan.method, plan.path, plan.body);
  return { gate, decision, apiCall, executed: true };
}
