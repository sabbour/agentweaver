...
const COL_GAP = DESIGN_TOKENS.geometry.columnGap;
const ROW_GAP = DESIGN_TOKENS.geometry.rowGap;
const BAND_GAP_MIN = 58;
const GAP_PAD = 28;
const GROUP_PAD_SIDE = DESIGN_TOKENS.geometry.groupPaddingX;
const GROUP_PAD_TOP = DESIGN_TOKENS.geometry.groupPaddingTop;
const GROUP_PAD_BOTTOM = DESIGN_TOKENS.geometry.groupPaddingBottom;
const CANVAS_MARGIN = DESIGN_TOKENS.geometry.canvasMargin;
const LANE_STEP = 34;
const LABEL_CHAR_W = DESIGN_TOKENS.typography.edgeSize * 0.567;
const LABEL_PAD_X = 20;
const LABEL_LINE_H = DESIGN_TOKENS.typography.edgeSize * 1.15;
...
function layoutGroupOverview(spec, groups) {
    ...
    const canvasHeight = brandHeight + rows * groupHeight + Math.max(rows - 1, 0) * gap + margin * 2 - (compact ? 2 : 0);
    const nodes = [];
...
function layerNodes(nodes, edges, outerRank) {
    const byId = new Map(nodes.map((n) => [n.id, n]));
    ...
    for (const e of edges ?? []) {
        if (!byId.has(e.from) || !byId.has(e.to))
            continue;
        if (rank.get(e.from) < rank.get(e.to))
            preds.get(e.to).push(e.from);
    }
    ...
    const layers = levels.map(() => []);
    for (const n of nodes)
        layers[indexOf.get(rank.get(n.id) ?? 0)].push(n);
    const slot = new Map();
...
function snakeColsFor(n, budget) {
...
function serpentine(chain, cols) {
    const out = [];
...
export function findLoopbackReturnJoinNode(loopback, edges) {
...
export function layout(spec) {
    ...
    const meanRank = (ns) => ns.reduce((a, n) => a + (rank.get(n.id) ?? 0), 0) / Math.max(ns.length, 1);
    const bands = [];
    ...
    const noteGutter = (i, labelH) => {
        if (i < 0 || i >= gutterLanes.length)
            return;
    ...
    const rowKey = (bi, r) => `${bi}-${r}`;
    const laneCountRow = (bi, r) => rowLanes.get(rowKey(bi, r)) ?? 0;
    ...
    const lateralSideUsed = new Set();
    (spec.edges ?? []).forEach((e, idx) => {
        const sb = bandOf.get(e.from);
        const tb = bandOf.get(e.to);
        if (sb === undefined || sb !== tb)
            return;
        ...
        if (lateralSideUsed.has(sKey) || lateralSideUsed.has(tKey))
            return;
    ...
    const labelGeom = new Map();
    (spec.edges ?? []).forEach((e, idx) => {
        if (!e.label)
            return;
        labelGeom.set(idx, measureLines(lateral.has(idx) ? wrapLabel(e.label, LATERAL_LABEL_MAX_W) : [e.label]));
    });
    ...
    const bandColGap = bands.map((_, bi) => {
        let widest = 0;
        (spec.edges ?? []).forEach((e, idx) => {
            if (!lateral.has(idx) || bandOf.get(e.from) !== bi)
                return;
    ...
    const rowLabelH = new Map();
    const stepGutter = (i) => Math.max(LANE_STEP, (gutterLabelH[i] ?? 0) + 10);
    const stepRow = (bi, r) => Math.max(LANE_STEP, (rowLabelH.get(rowKey(bi, r)) ?? 0) + 10);
    (spec.edges ?? []).forEach((e, idx) => {
        const sb = bandOf.get(e.from);
        const tb = bandOf.get(e.to);
        if (sb === undefined || tb === undefined)
            return;
    ...
    const inners = grids.map((g, bi) => {
        ...
        const padBottom = bands[bi].id
            ? Math.max(GROUP_PAD_BOTTOM, laneCountRow(bi, g.rows - 1) * stepRow(bi, g.rows - 1) + GAP_PAD)
            : 0;
        return {
            ...g,
            colGap,
    ...
    const cardSpan = new Map();
    bands.forEach((band, bi) => {
        const inner = inners[bi];
        band.nodes.forEach((n, i) => {
            const x = rowOriginX(bi, Math.floor(i / inner.cols)) +
    ...
    const gutterRuns = gutterLanes.map(() => []);
    const addRun = (g, idx, a, b) => {
        if (g < 0 || g >= gutterRuns.length)
            return;
    ...
    (spec.edges ?? []).forEach((e, idx) => {
        const sb = bandOf.get(e.from);
        const tb = bandOf.get(e.to);
        if (sb === undefined || tb === undefined || sb === tb)
            return;
    ...
    const gapFor = (i) => Math.max(BAND_GAP_MIN, (gutterLanes[i] ?? 0) * stepGutter(i) + GAP_PAD);
    // Vertical span available to in-band runs, per row gutter.
    const rowGutter = new Map();
    ...
    (spec.edges ?? []).forEach((e, idx) => {
        ...
        if (e.loopback)
            ...
            kind = lateral.has(idx) ? 'lateral' : 'same';
        else if (Math.abs(t.band - s.band) > 1)
            kind = t.band > s.band ? 'sideDown' : 'sideUp';
        else
            kind = t.band > s.band ? 'down' : 'up';
        const sSide = kind === 'up' || kind === 'sideUp' ? 'top' : 'bottom';
        const tSide = kind === 'same' || kind === 'lateral'
            ? 'bottom'
            : kind === 'up' || kind === 'sideUp'
                ? 'bottom'
                : 'top';
    ...
    const ports = new Map();
    const portKey = (nodeId, side) => `${nodeId}::${side}`;
    ...
    const loopbackRails = new Map();
    const loopbackRail = (returnJoin, side) => {
    ...
    const verticals = [];
    const loopbackLabelOwners = new Set();
    const labelledReturnFamilies = new Set();
    for (const route of routed) {
        if (!route.e.loopback || !route.e.label)
            continue;
        const key = `${findLoopbackReturnJoinNode(route.e, spec.edges)}\0${route.e.label}`;
        if (labelledReturnFamilies.has(key))
            continue;
    ...
    for (const r of routed) {
        ...
        const runEndX = kind === 'lateral' ? points[1].x : kind === 'sideDown' || kind === 'sideUp' ? points[2].x : tx;
    ...
    for (const edge of rfEdges) {
        ...
        if (data?.loopback || outgoingBySource.has(edge.source))
            continue;
        outgoingBySource.set(edge.source, edge);
    }
    for (const edge of rfEdges) {
        const data = edge.data;
        if (!data?.loopback || !data.points)
            continue;
        ...
        const points = alignLoopbackToContinuation(data.points, continuationPoints, target
    ...
    for (const box of order) {
        ...
        for (let x = lo; x <= hi; x += 3)
            candidates.push(x);
        candidates.push(hi);
        if (box.ideal >= lo && box.ideal <= hi)
            candidates.push(box.ideal);
        let bestX = box.ideal;
        ...
        for (const x of candidates) {
            ...
            for (const v of verticals) {
                if (v.edgeId === box.edgeId)
                    continue;
                if (v.x < left || v.x > right)
                    continue;
                if (v.y1 < top || v.y0 > bottom)
                    continue;
                crossings += 1;
            }
            let collisions = 0;
            for (const p of settled) {
                if (Math.abs(p.y - box.y) >= (p.h + box.h) / 2 + 4)
                    continue;
                if (Math.abs(p.x - x) >= (p.w + box.w) / 2 + 10)
                    continue;
                collisions += 1;
            }
    ...
    for (const edge of rfEdges) {
        ...
        const data = edge.data;
        if (data?.labelPos)
            data.labelPos = { x: fixed.x, y: fixed.y };
    }
    ...
    for (const group of groups) {
        if (!group.parent || !groupById.has(group.parent))
            continue;
        if (!children.has(group.parent))
            children.set(group.parent, []);
        children.get(group.parent).push(group.id);
    }
    const bandBox = new Map();
    for (const band of bands) {
        if (band.id)
            bandBox.set(band.id, { x: band.x, y: band.y, w: band.w, h: band.h });
    }
    const groupBoxes = new Map();
    const resolving = new Set();
    const resolveGroupBox = (id) => {
        const cached = groupBoxes.get(id);
        if (cached)
            return cached;
        if (resolving.has(id))
            return bandBox.get(id);
        resolving.add(id);
        const parts = [
            ...(bandBox.has(id) ? [bandBox.get(id)] : []),
            ...(children.get(id) ?? []).map(resolveGroupBox).filter((box) => box !== undefined),
        ];
        resolving.delete(id);
        if (parts.length === 0)
            return undefined;
    ...
    const belongsToGroup = (nodeId, groupId) => {
    ...
    const bypassLaneByEdge = new Map();
    for (const edge of rfEdges) {
        const data = edge.data;
    ...
    const finalJunctionsByEdge = findConnectorJunctions(rfEdges.map((edge) => ({
        ...
        source: edge.source,
        target: edge.target,
        loopback: edge.data?.loopback,
        returnJoin: edge.data?.returnJoin,
        points: (edge.data?.points ?? []),
    })));
    for (const edge of rfEdges) {
        const data = edge.data;
    ...
    if (renderedBoxes.length > 0) {
        ...
        canvasWidth = Math.max(canvasWidth, ...[...groupBoxes.values()].map((box) => box.x + box.w + CANVAS_MARGIN));
        canvasHeight = Math.max(canvasHeight, ...[...groupBoxes.values()].map((box) => box.y + box.h + CANVAS_MARGIN));
    }
