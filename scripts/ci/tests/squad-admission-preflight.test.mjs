...
test('non-local CLI materializes and preflights through an explicit runtime adapter', async () => {
  const teamRoot = await mkdtemp(join(tmpdir(), 'agentweaver-runtime-state-'));
  const adapterPath = join(teamRoot, 'adapter.mjs');
  const inputPath = join(teamRoot, 'input.json');
  await writeFile(adapterPath, `
    import { mkdir, readFile, rename, writeFile } from 'node:fs/promises';
    import { dirname, join } from 'node:path';
    export async function createStateAdapter({ teamRoot }) {
      return {
        async writeAtomic(key, value) {
          const path = join(teamRoot, 'runtime-state', key);
        ...
        read(key) {
          return readFile(join(teamRoot, 'runtime-state', key), 'utf8');
        }
      };
    }
  `);
  await writeFile(inputPath, JSON.stringify(ledger()));
  const cwd = fileURLToPath(new URL('../../../', import.meta.url));
  const materializer = fileURLToPath(new URL('../squad-admission-ledger.mjs', import.meta.url));
  const preflight = fileURLToPath(new URL('../squad-admission-preflight.mjs', import.meta.url));
  await execFileAsync(process.execPath, [
    materializer,
    '--input', inputPath,
    '--team-root', teamRoot,
    '--state-backend', 'two-layer',
    '--state-adapter', adapterPath,
  ], { cwd });
  const { stdout } = await execFileAsync(process.execPath, [
    preflight,
    expected.repository,
