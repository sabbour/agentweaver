...
test('admission is external-state owned and squash-only', () => {
  ...
  const materializer = read('scripts/ci/squad-admission-ledger.mjs');
  const common = read('scripts/ci/squad-admission-common.mjs');
  ...
  assert.match(contract, /squad-admission-ledger\.mjs materialize <owner\/repository> <pr-number>/u);
  ...
  assert.match(contract, /does not provide\s+an\s+adversarially immutable execution boundary/u);
  assert.doesNotMatch(validator, /Active squad:\s*external/u);
  assert.doesNotMatch(contract, /npm run squad:admission-preflight/u);
  assert.match(materializer, /const allowed = new Set\(\['--head-sha', '--input-file', '--replace-existing-head'\]\)/u);
  assert.match(common, /resolveSquadDirectory \?\? sdk\.resolveSquad/u);
  assert.match(common, /resolveDirectory \?\? sdk\.resolveExternalStateDir/u);
  assert.match(common, /projectKey, false/u);
