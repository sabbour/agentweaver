...
test('admission is external-state owned and squash-only', () => {
  ...
  assert.match(contract, /Coordinator\/Ralph.*host, Git, and the configured remote.*trusted operational components/su);
  ...
  assert.equal(existsSync(new URL('scripts/ci/squad-admission-launcher.mjs', root)), true);
  assert.match(contract, /not a tamper-proof sandbox/u);
  assert.match(contract, /does not provide an adversarially immutable execution boundary|does not claim\s+immutable protection against a compromised coordinator or host/u);
  ...
  assert.match(contract, /agentweaver\.squad-admission-findings\/v3/u);
  assert.match(contract, /#1504 cannot use its candidate-owned v3 source to admit itself|PR #1504 may use only the pre-existing v1\/manual/u);
  assert.match(contract, /mandatory\s+for every subsequent PR/u);
  assert.match(contract, /while the PR is still draft/u);
  assert.match(contract, /squad-admission-launcher\.mjs/u);
  assert.match(contract, /Git object ID/u);
  assert.match(contract, /aggregate digest/u);
  assert.match(contract, /source ref\/commit|source ref and commit/u);
  assert.match(contract, /There is no install step|There is no\s+installation step/u);
  assert.match(contract, /compromised host/u);
  assert.doesNotMatch(contract, /admission\/runtime|runtime-manifest/u);
  assert.match(launcher, /trusted launcher bytes do not match the fetched base object/u);
  assert.match(launcher, /ephemeral bytes for .* do not match the fetched base object/u);
  assert.match(launcher, /finally/u);
  assert.match(validator, /candidate checkout admission code is evidence only/u);
