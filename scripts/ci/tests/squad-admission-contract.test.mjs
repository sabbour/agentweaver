...
test('admission is external-state owned and squash-only', () => {
  ...
  assert.match(contract, /mandatory for every subsequent PR/u);
  assert.match(contract, /while the PR is still draft/u);
  assert.match(contract, /squad-admission-launcher\.mjs/u);
  assert.match(contract, /launcher digest|launcherDigest/u);
  assert.match(contract, /policy digest|policyDigest/u);
  assert.match(contract, /trusted source ref\/commit|source ref and commit/u);
  assert.match(launcher, /runtime launcher digest does not match/u);
  assert.match(launcher, /runtime policy digest does not match/u);
