...
public sealed class CastingWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    public CastingWebApplicationFactory()
    {
        ...
        _dbPath          = Path.Combine(Path.GetTempPath(), $"scaffolder-cast-{unique}.db");
        _workspaceRoot   = Path.Combine(Path.GetTempPath(), $"scaffolder-cast-ws-{unique}");
        _worktreesPath   = Path.Combine(Path.GetTempPath(), $"scaffolder-cast-wt-{unique}");
        _checkpointsPath = Path.Combine(Path.GetTempPath(), $"scaffolder-cast-cp-{unique}");
