...
public sealed class CommitEndpointMergeTests : IClassFixture<ReviewWebApplicationFactory>, IDisposable
{
    ...
    public async Task Commit_MergeConflict_ReturnsMergeFailed_WithConflictFiles()
    {
        ...
        // The worktree must be removed (MergeFailed is cleanly terminal; conflict info is in the DB).
        Directory.Exists(run.WorktreePath).Should().BeFalse(
            "the worktree must be removed on merge conflict — conflict info is stored in merge_conflicts");
    ...
    /// <summary>
    /// Creates a real git repository, adds a worktree for a scaffolder run,
    ...
    private string CreateTempGitRepo()
    {
        var repoPath = Path.Combine(
            Path.GetTempPath(), $"scaffolder-commit-test-{Guid.NewGuid():N}");
