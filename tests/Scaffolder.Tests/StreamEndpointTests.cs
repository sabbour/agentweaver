...
/// </summary>
public sealed class StreamEndpointTests : IClassFixture<ScaffolderWebApplicationFactory>
{
    private readonly ScaffolderWebApplicationFactory _factory;
    private readonly HttpClient _ownerClient;

    public StreamEndpointTests(ScaffolderWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    public async Task Owner_CanStream_InProgressRun()
    {
        ...
        var streamStore = _factory.Services.GetRequiredService<RunStreamStore>();
        var runId = Guid.NewGuid().ToString();
        var entry = streamStore.Create(runId, ScaffolderWebApplicationFactory.TestUser);
