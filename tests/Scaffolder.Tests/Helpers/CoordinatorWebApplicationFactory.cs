...
public sealed class CoordinatorWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    public CoordinatorWebApplicationFactory()
    {
        ...
        _dbPath                     = Path.Combine(Path.GetTempPath(), $"scaffolder-coord-{unique}.db");
        _workspaceRoot              = Path.Combine(Path.GetTempPath(), $"scaffolder-coord-ws-{unique}");
        _worktreesPath              = Path.Combine(Path.GetTempPath(), $"scaffolder-coord-wt-{unique}");
        _checkpointsPath            = Path.Combine(Path.GetTempPath(), $"scaffolder-coord-cp-{unique}");
        _coordinatorCheckpointsPath = Path.Combine(Path.GetTempPath(), $"scaffolder-coord-ccp-{unique}");
