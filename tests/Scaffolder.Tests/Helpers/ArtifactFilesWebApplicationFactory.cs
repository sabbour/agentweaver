...
public sealed class ArtifactFilesWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    {
        _dbPath          = Path.Combine(Path.GetTempPath(), $"scaffolder-af-{Guid.NewGuid():N}.db");
        _worktreesPath   = Path.Combine(Path.GetTempPath(), $"scaffolder-af-wt-{Guid.NewGuid():N}");
        _checkpointsPath = Path.Combine(Path.GetTempPath(), $"scaffolder-af-cp-{Guid.NewGuid():N}");
    }
