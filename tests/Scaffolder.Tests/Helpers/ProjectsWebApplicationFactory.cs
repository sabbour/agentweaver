...
public sealed class ProjectsWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    public ProjectsWebApplicationFactory()
    {
        ...
        _dbPath          = Path.Combine(Path.GetTempPath(), $"scaffolder-proj-{unique}.db");
        _workspaceRoot   = Path.Combine(Path.GetTempPath(), $"scaffolder-proj-ws-{unique}");
        _worktreesPath   = Path.Combine(Path.GetTempPath(), $"scaffolder-proj-wt-{unique}");
        _checkpointsPath = Path.Combine(Path.GetTempPath(), $"scaffolder-proj-cp-{unique}");
