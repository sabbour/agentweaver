...
public sealed class CoordinatorAssemblyStoreTests : IDisposable
{
    ...
    public async Task TryStartAssembly_ReturnsTrueOnce_FalseOnSecondCall()
    {
        ...
        var first = await _sut.TryStartAssemblyAsync(workPlanId, "scaffolder/integration/x", default);
        var second = await _sut.TryStartAssemblyAsync(workPlanId, "scaffolder/integration/x", default);
        ...
        state.IntegrationBranch.Should().Be("scaffolder/integration/x");
    }
    ...
    public async Task TryStartAssembly_ReturnsFalse_WhenNotAwaitingAssembly()
    {
        ...
        var result = await _sut.TryStartAssemblyAsync(workPlanId, "scaffolder/integration/x", default);
    ...
    public async Task TryStartAssembly_ConcurrentCallers_ExactlyOneWins()
    {
        ...
        var tasks = Enumerable.Range(0, 16)
            .Select(_ => Task.Run(() => _sut.TryStartAssemblyAsync(workPlanId, "scaffolder/integration/x", default)))
            .ToArray();
