...
public sealed class SandboxEscapeEndToEndTests
{
        ...
        ModelSource provider, IAgentRunner runner, IReadOnlyList<string> logLines)
    {
        ...
        var sandbox = Path.Combine(Path.GetTempPath(), $"agentweaver-e2e-sbx-{Guid.NewGuid():N}");
        ...
        var canaryPath = Path.Combine(Path.GetTempPath(), $"agentweaver-e2e-canary-{Guid.NewGuid():N}.txt");
    ...
    private static string? FindRepoApiDevSettings()
    {
        ...
        while (dir is not null)
        {
            var candidate = Path.Combine(
                dir.FullName, "apps", "Agentweaver.Api", "appsettings.Development.json");
