...
public sealed class SandboxPolicyBackendTests : IDisposable
{
    ...
    public void StartPreviewProcess_AbsoluteSandboxRootCwd_Allowed()
    {
        ...
        decision.Allowed.Should().BeTrue();
        decision.Metadata!["resolved_directory"].Should().Be(Path.GetFullPath(_sandboxRoot));
    }
    ...
    public void StartPreviewProcess_AbsoluteSubdirectoryCwdInsideSandbox_Allowed()
    {
        ...
        decision.Allowed.Should().BeTrue();
        decision.Metadata!["resolved_directory"].Should().Be(Path.GetFullPath(insidePath));
    }
    ...
    [Fact]
    public void StartPreviewProcess_AbsoluteTraversalOutsideSandbox_Denied()
    {
        var context = new Dictionary<string, object>
        {
            ["tool_name"] = "start_preview_process",
            ["cwd"] = Path.Combine(_sandboxRoot, "..", "outside"),
        };
