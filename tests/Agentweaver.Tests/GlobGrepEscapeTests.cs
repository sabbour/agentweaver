...
public sealed class GlobGrepEscapeTests
{
    ...
    public async Task GitHubCopilot_GlobEscape_CanaryNeverLeaks()
    {
        ...
        var parentDir = Path.Combine(Path.GetTempPath(), $"agentweaver-glob-parent-{Guid.NewGuid():N}");
    ...
    private static string? FindRepoApiDevSettings()
    {
        ...
        {
            var candidate = Path.Combine(dir.FullName, "apps", "Agentweaver.Api", "appsettings.Development.json");
